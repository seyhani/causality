...
class TestEventStructure(unittest.TestCase):
    ...
    def test_product_prefix(self):
        p = EventStructure().prefix('b').prefix('a').product(EventStructure().prefix('y').prefix('x'))
    ...
    def test_restrict(self):
        s = EventStructure().prefix('a').sum(EventStructure().prefix('b'))
        self.assertIn((1, 0, 'b'), ids(s.events))
        self.assertIn((1, 0, 'b'), ids(s.get_conflict((0, 0, 'a'))))
        s = s.restrict({'a'})
    ...
    def test_relabeling(self):
        p = EventStructure().prefix('a').product(EventStructure().prefix('b'))
        p = p.relabel({('a', '*'): 'x', ('*', 'b'): 'y'})
