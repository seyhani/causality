...
\begin{align*}
    ...
    \forall k'. s_k \subset s_{k'} \Rightarrow E_{k',i}
    \right)                                           \\
                   & \wedge \left(
    M_{k,i} \Rightarrow \forall k'.
    (s_k \subset s_{k'} \vee s_{k'} \subset s_k) \Rightarrow \neg M_{k',i}
    \right)
\end{align*}
...
\begin{align*}
    ...
    Min(k,i)                              & = \neg \left(
    \bigvee_{
        \forall k'. s_{k} \subset s_{k'} \wedge e_i \notin{s_{k'}}
    } M_{k',i}
    \right)                                                                          \\
...
\subsection{Examples}
\newcommand{\f}[1]{F_{#1}(\vec u,\vec V_{#1})}
\newcommand{\e}{\emptyset}
\newcommand{\m}{(M,\vec u)}
\begin{example}
    Consider the following network.
    Assume that we model the arrival of a packet on the switch H1 and H2 with events s, $h_1$, and $h_2$ respectively.
    Consider the configuration $\sigma = \s{a,b}$.
    We can define the event structure using the enabling relation:
    \begin{align*}
        \e \vdash_{min} h_1, \e \vdash_{min} h_2
    \end{align*}
    And an empty conflict relation.
    Consider the permutation $\pi = a,b$.
    ...
    \begin{align*}
        \varphi_{\sigma} & =
        \left(
        E(\e, a) \wedge E(\s{a},b) \vee E(\e,b) \wedge E(\s{b},a)
        \right)
        \wedge Con(\sigma)                                                       \\
        C(a,b)           & = CE(a,b)                                             \\
        ...
        Min(\s{b}, a)    & = \neg M(\e, a)                                       \\
    \end{align*}
    Regarding these functions we have:
    \begin{align*}
        (M,\vec u) \vDash \varphi_{\sigma} = true
    \end{align*}
    Now let's add a conflict between $a$ and $b$, thus we manually set
    $C(a,b) = true$.
    Thus we have $Con(\s{a,b}) = \neg C(a,b)$ if we set $C(a,b) = true$, then $Con(\s{a,b})$ becomes false and subsequently $\varphi_{\sigma}$ becomes false.
    This we have:
    \begin{align*}
        (M,\vec u) \vDash [C(a,b) \leftarrow true] \neg \varphi_{\sigma}
    \end{align*}
    Thus we can consider $C(a,b)$ as a cause of $\varphi_{\sigma}$
    using the witness $(\emptyset, \emptyset, true)$.
    Since the set $\vec W$ in the witness is empty, $C(a,b)$ is a but-for cause and thus we do not need to verify the AC2(b) condition.
\end{example}
...
\begin{example}
    ...
    If we assume the enabling relation as follows:
    \begin{align*}
        \e \vdash i, \e \vdash o
    \end{align*}
    ...
    Thus we have:
    \begin{align*}
        \m \vDash \varphi_{\sigma}
    \end{align*}
    Now we can introduce $M(\s{i},o) = false$ as a cause of $\varphi_{\sigma}$.
    Assume that we set $M(\s{i},o) = true$.
    So, $Min(\e, o)$ becomes false and thus $M(\e, o)$ becomes false.
    Thus we have:
    \begin{align*}
        (M, \vec u) \vDash [M(\s{i},e) \leftarrow true] \neg \varphi_{\sigma}
    \end{align*}
\end{example}
