\begin{example}
    ...
    \begin{align*}
        \varphi & = E(\e,o) \wedge Con(\e) \\
                & = E(\e,o)
    \end{align*}
    Now we can consider $M(\e,o)$ as a cause of $\varphi$.
    If we set $M(\e,o)$ to false, then $E(\e,o)$ becomes false and finally
    $\varphi$ becomes false.
    Since we have considered an empty $\vec W$ and the AC2(a) condition
    is satisfied, thus $M(\e,o) = \F$ is a but-for cause of $\varphi$.
...
\begin{example}
    ...
    \end{itemize}
    For verifying the AC2(a) condition notice that setting $M(\s{b},i)$ and
    $M(\s{c},i)$ to false results in $E(\s{b},i)$ and $E(\s{c},i)$ become false.
    ...
    Also note that $E(\s{b},i)$ is true in the actual context, it remains
    true by setting any $\vec Z'$ to $\vec z^*$.
    Thus, the disjunction of $E(\s{b,c},i)$ has a true value and thus it
    remains true even if we set $E(\s{c},i)$ to false.
    So $\varphi$ remains true and AC2(b) is satisfied.
\end{example}

\begin{example}
    Consider the following network.
    We wish to forward traffic from $a$ to $b$ and $x$ to $y$.
    But we also want to limit the traffic on the link 3 so at any moment
    ...
    We define an event $c$ when congestion is detected on link 3 (at least two packets are being traversed through the).
    Thus, we define the conflict relation in such a way that we have:
    \begin{align*}
        a \# y, x\# b
    \end{align*}
    ...
    Since we have used an empty $\vec W$, and we have satisfied the AC2(a)
    condition, so $M(\e,x) = \T$ is a but-for cause of $\varphi$.
\end{example}
