\begin{example}
    ...
    \begin{align*}
        ...
        \m & \vDash Con(i,o) = \T    \\
    \end{align*}
    Let $\vec v$ be the values of the endogenous variables in the actual
    context and $\mathrm{E} = \mathfrak{E}(\vec v) = (\s{i,o},\e,\vdash)$ where we have:
    \begin{align*}
        \e \vdash o, \s{i} \vdash o, \e \vdash i, \s{o} \vdash i
    \end{align*}
    Now we consider $\sigma = \s{o}$ as a counterexample using 
    $(\e,\e,\F)$ as a witness.
    ...
    so these are the only variables that are affected by changing $M(\e,o)$.
    Thus, we have:
    \begin{align*}
